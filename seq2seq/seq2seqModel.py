...
class DecoderRNN(nn.Module):
    def __init__(self, vocab_size, hidden_size, n_layers=2, bias = True):
...
class EncoderRNNattention(nn.Module):
    def __init__(self, input_size, hidden_size, n_latent, n_layers=2, bias = True, expand = False):
...
class Seq2Seq(nn.Module):
    ...
    def train(self, input_variable, target_variable, criterion, criterionDicrim, teacher_forcing_ratio, args, test = False, profForc = False, professor_forcing_ratio = 0):
        ...
        for t in range(int(self.lenPred/self.decim)):
            ...
            if profForcBatch != True:
                loss += criterion(decoder_output[0,:,:].view(batch_size,self.decoder.vocab_size), target_variable[:,t,:])
            else:
            ...
            if teachForcBatch :
                decoder_output = target_variable[:,t,:].view(batch_size,1,self.decoder.vocab_size)
